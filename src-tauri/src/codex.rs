...
pub struct ThemeRuntime {
    ...
    idle: std::sync::Arc<std::sync::Mutex<Option<IdleInstance>>>,
...
impl ThemeRuntime {
    ...
    fn take_idle_instance(&self) -> Result<Option<IdleInstance>, CodexError> {
        self.idle
            .lock()
            .map_err(|_| CodexError("主题会话状态已损坏".into()))
    ...
    fn park_idle_instance(&self, instance: TestInstance, port: u16) -> Result<(), CodexError> {
        let mut state = self
            .idle
            .lock()
            .map_err(|_| CodexError("主题会话状态已损坏".into()))?;
        *state = Some(IdleInstance { instance, port });
        Ok(())
    }
}
...
impl WindowsStoreProcess {
    ...
    fn open_attached(id: u32) -> Result<Self, CodexError> {
        let mut process = Self::open(id)?;
        process.terminate_on_drop = false;
        Ok(process)
    }
...
fn start_theme_target(
    installation: &CodexInstallation,
) -> Result<(TestInstance, u16, bool, DevToolsTarget), CodexError> {
    let mut instance = start_theme_instance(installation)?;
    let profile_path = instance.profile_path().to_path_buf();
    let (port, _) = wait_for_devtools(&profile_path, &mut instance.process)?;
    let socket = SocketAddr::new(IpAddr::V4(Ipv4Addr::LOCALHOST), port);
    let loopback_only = verify_loopback(socket, instance.process.id())?;
    let target = wait_for_codex_target(port, &mut instance.process)?;
    Ok((instance, port, loopback_only, target))
}

fn start_theme_instance(installation: &CodexInstallation) -> Result<TestInstance, CodexError> {
    #[cfg(target_os = "windows")]
    {
        return start_existing_windows_instance(installation);
    }

    #[cfg(not(target_os = "windows"))]
    {
        start_isolated(installation)
    }
}

#[cfg(target_os = "windows")]
fn windows_codex_profile(installation: &CodexInstallation) -> Result<PathBuf, CodexError> {
    let mut candidates = Vec::new();
    if let Some(local_app_data) = std::env::var_os("LOCALAPPDATA") {
        candidates.push(
            PathBuf::from(local_app_data)
                .join("Packages")
                .join(&installation.bundle_id)
                .join("LocalCache")
                .join("Roaming")
                .join("Codex")
                .join("web")
    ...
    candidates
        ...
        .ok_or_else(|| CodexError("无法找到当前 ChatGPT 的用户数据目录".into()))
}

#[cfg(target_os = "windows")]
fn start_existing_windows_instance(
    installation: &CodexInstallation,
) -> Result<TestInstance, CodexError> {
    let profile_path = windows_codex_profile(installation)?;
    if !profile_path.join("DevToolsActivePort").is_file() {
        return Err(CodexError("RETHEME_CHATGPT_DEBUG_REQUIRED".into()));
    }
    let process = activate_windows_store_app_attached(&installation.app_user_model_id, "")?;
    Ok(TestInstance {
...
fn start_theme_package_preview(
    ...
    if runtime.current_preview()?.is_some() {
        return replace_theme_package_preview(
            runtime,
            compatibility,
            package,
            source,
    ...
    let runtime_config = package
        ...
        .map_err(|error| CodexError(error.to_string()))?;
    let idle = runtime.take_idle_instance()?;
    let (instance, port, loopback_only, target, reused_idle) = if let Some(mut idle) = idle {
        if idle.instance.process.has_exited()? {
            drop(idle);
            let (instance, port, loopback_only, target) = start_theme_target(&installation)?;
            (instance, port, loopback_only, target, false)
        } else {
            match find_codex_target(idle.port, STATUS_PROBE_TIMEOUT) {
                Ok(Some(target)) => (idle.instance, idle.port, true, target, true),
                _ => {
                    drop(idle);
                    let (instance, port, loopback_only, target) =
                        start_theme_target(&installation)?;
                    (instance, port, loopback_only, target, false)
                }
            }
        }
    ...
    let applied_slots = match apply_theme(
        ...
        Ok(applied_slots) => applied_slots,
        Err(error) => {
            if reused_idle {
                let _ = runtime.park_idle_instance(instance, port);
            }
            return Err(error);
        }
    };
...
fn replace_theme_package_preview(
    runtime: &ThemeRuntime,
    compatibility: &compatibility::CompatibilityRepository,
    package: theme::ThemePackage,
    source: ThemePreviewSource,
    ...
    let previous = {
        ...
        state
            .take()
            .ok_or_else(|| CodexError("没有正在运行的主题可切换".into()))?
    };
    let previous_port = previous.port;
    let remaining = expires_at.map(remaining_until).transpose()?.flatten();
    let installation = detect()?;
    let adapter = compatibility.adapter(&installation.version);
    let target = find_codex_target(previous_port, STATUS_PROBE_TIMEOUT)?
        .ok_or_else(|| CodexError("ChatGPT 主题页面已退出，请重新打开 ChatGPT".into()))?;
    let session_id = runtime.next_session_id();
    let (asset_server, asset_urls, revoke_assets_url) =
        ThemeAssetServer::start(package.runtime_assets())?;
    let runtime_config = package
    ...
    let ThemeSession { instance, .. } = previous;
    let mut state = runtime
        .session
        .lock()
        .map_err(|_| CodexError("主题会话状态已损坏".into()))?;
    *state = Some(ThemeSession {
        id: session_id,
        instance,
        _asset_server: Some(asset_server),
        port: previous_port,
...
pub fn stop_theme_preview(runtime: &ThemeRuntime) -> Result<bool, CodexError> {
    ...
    match result {
        Ok(removed) => {
            let ThemeSession { instance, port, .. } = session;
            runtime.park_idle_instance(instance, port)?;
            Ok(removed)
        }
        Err(error) => {
            drop(session);
...
fn start_isolated(installation: &CodexInstallation) -> Result<TestInstance, CodexError> {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        return Ok(TestInstance {
            process: IsolatedProcess::Native { child },
            _profile: profile,
            profile_path,
            terminate_on_drop: true,
        });
...
fn activate_windows_store_app(
    ...
    activate_windows_store_app_with_ownership(app_user_model_id, arguments, true)
}

#[cfg(target_os = "windows")]
fn activate_windows_store_app_attached(
    app_user_model_id: &str,
    arguments: &str,
) -> Result<WindowsStoreProcess, CodexError> {
    activate_windows_store_app_with_ownership(app_user_model_id, arguments, false)
}

#[cfg(target_os = "windows")]
fn activate_windows_store_app_with_ownership(
    app_user_model_id: &str,
    arguments: &str,
    terminate_on_drop: bool,
) -> Result<WindowsStoreProcess, CodexError> {
    ...
    if terminate_on_drop {
        ...
        WindowsStoreProcess::open_attached(process_id)
    }
}
